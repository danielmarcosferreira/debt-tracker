...
import { cardBalance, cardTotalsInScope, type CardMonthTotal, type CurrencyTotals } from "@/lib/aggregates";
...
export default function CardsPage() {
  ...
  const [tab, setTab] = useState<"cards" | "invoices">("cards");
  ...
  const limitTotals = useMemo(() => {
    ...
    for (const c of cards) {
      if (!c.limit) continue;
      totals[c.currency] = (totals[c.currency] ?? 0) + c.limit;
    }
    return totals;
  }, [cards]);
  ...
  const availableTotals = useMemo(() => {
    ...
    for (const c of cards) {
      if (!c.limit) continue;
      totals[c.currency] = (totals[c.currency] ?? 0) + Math.max(0, c.limit - cardBalance(c.id, expenses));
    }
    return totals;
  }, [cards, expenses]);
  ...
  return (
    <>
      ...
      <main className="px-2 pt-5">
        <div className="mb-4 flex w-full items-center gap-0.5 rounded-full bg-slate-100 p-0.5 dark:bg-slate-800">
          {(["cards", "invoices"] as const).map((tabKey) => (
            <button
              key={tabKey}
              type="button"
            ...
            >
              {tabKey === "cards" ? t("cards.tabCards") : t("cards.tabInvoices")}
            </button>
          ))}
        </div>
        ...
        {tab === "cards" ? (
          <>
            {Object.keys(limitTotals).length > 0 && (
              <div className="mb-4 grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                    {t("cards.totalLimit")}
                  </p>
                  <p className="mt-1.5 text-lg font-semibold tabular-nums text-slate-900 dark:text-slate-100">
                    <TotalsLine totals={limitTotals} fallback={currency} />
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                    {t("cards.totalAvailable")}
                  </p>
                  <p className="mt-1.5 text-lg font-semibold tabular-nums text-slate-900 dark:text-slate-100">
                    <TotalsLine totals={availableTotals} fallback={currency} />
                  </p>
                </div>
              </div>
            )}
